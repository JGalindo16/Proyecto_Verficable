...
class CerrarSeccionYReportesService:
    ...
    def cerrar_seccion(self, section_id: int):
        try:
            ...
            for row in notas:
                self.cursor.execute(q.GET_STUDENT_ID_FROM_ENROLLMENT, (row["enrollment_id"],))
                student = self.cursor.fetchone()
                if not student:
                    continue

                nota = self._normalizar_nota(row["nota_final"])
                self.cursor.execute(q.INSERT_NOTA_FINAL, (section_id, student["student_id"], round(nota, 2)))
    ...
    def generar_reporte_resumen_por_estudiante(self, student_id: int):
        try:
            ...
            cursos = defaultdict(lambda: {
                "nombre": "", "codigo": "", "evaluaciones": defaultdict(lambda: {"type": "", "weight": 0.0, "instancias": []})
            })
